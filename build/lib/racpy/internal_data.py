...
tissues = ["adipose_tissue", "adrenal_gland", "blood", "blood_vessel",
           "brain", "breast", "colon", "esophagus", "heart", "liver",
           "lung", "muscle", "nerve", "ovary", "pancreas", "pituitary",
           "prostate", "salivary_gland", "skin", "small_intestine",
           "spleen", "stomach", "testis", "thyroid", "uterus", "vagina",
           "all_tissues"]
signatures = ["DESeq", "Pearson", "Dev", "deMagalhaes", "GenAge", "Kuan_Ren",
              "Peters", "all"]
...
for t in tissues[:-1]:
    ...
    for sig in signatures:
        df = pd.read_csv(os.path.join(dataPath, "coef_" + t + "_" +
                                      sig + ".csv"))
...
for sig in signatures[1:]:
    df = pd.read_csv(os.path.join(dataPath, "coef_all_tissues_" +
                                  sig + ".csv"))
    coef_series = pd.Series(list(df.coef), index=df.geneid)
    genelist_element[sig] = coef_series
